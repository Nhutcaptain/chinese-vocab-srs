...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!apiKey) {
            return NextResponse.json({ error: "Missing OPENAI_API_KEY. Please add it to Vercel Environment Variables." }, { status: 500 });
        }

        const openai = new OpenAI({ apiKey });
        const { username, mode, topic, count = 5 } = await req.json();
